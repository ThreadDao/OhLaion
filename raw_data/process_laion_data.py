...
def get_the_eye_vector_file_names(data_url):
    ...
    for img_emb_a in img_emb_a_tags:
        all_file_names.append(pq(img_emb_a).attr('href'))
...
def process_data(vector_file_path, metadata_file_path, pk_start, dest_data_path):
    """
...
def loop_process_data(data_url, _source_data_dir, _dest_data_dir, exist_file_index, metadata_dir):
    """
    ...
    for cur_emb_file_name_index, cur_emb_file_name in enumerate(all_file_names):
        ...
        dest_data_path = f"{_dest_data_dir}/binary_768d_{cur_emb_file_name_index + exist_file_index:05d}.parquet"
        log.info(
        ...
        if check_file_exist(vector_file_path):
            next_pk_index = process_data(vector_file_path, metadata_file_path, next_pk_index, dest_data_path)
...
if __name__ == '__main__':
    ...
    data_url = "https://the-eye.eu/public/AI/cah/laion5b/embeddings/laion2B-multi/img_emb"
    metadata_dir = "laion2B-multi-metadata"
    loop_process_data(data_url, source_dir, dest_dir, exist_file_index, metadata_dir)